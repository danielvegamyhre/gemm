...
#ifdef DEBUG
                printf("tile_idx=%d, k=%d, bid_x=%d, bid_y=%d, tid=%d caching sA[%d, %d]\n", tile_idx, k, blockIdx.x, blockIdx.y, threadIdx.x, a_smem_row, a_smem_col);
#endif 
            }
...
#ifdef DEBUG
                printf("tile_idx=%d, k=%d, bid_x=%d, bid_y=%d, tid=%d caching sB[%d, %d]\n", tile_idx, k, blockIdx.x, blockIdx.y, threadIdx.x, b_smem_row, b_smem_col);
#endif 
            }
...
#ifdef DEBUG 
            printf("storing C[%d, %d] = %f from c_row=%d, tm=%d\n", c_row + tm, c_col + tn, thread_resultjs[tm][tn], c_row, tm);
#endif 
        }
    }
}
