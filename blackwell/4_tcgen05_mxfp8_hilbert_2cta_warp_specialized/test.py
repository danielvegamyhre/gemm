...
def test_gemm(M, K, N):
    ...
    print(f"Rows with unexpected zeros: {(unexpected_zeros_per_row > 0).sum().item()}/{M}")
    ...
    for i in range(col_chunks):
        ...
        print(f"Cols [{start_col:4d}-{end_col:4d}): mismatches={chunk_mismatches:6d}, unexpected_zeros={chunk_unexpected_zeros:6d}")
    ...
    if num_mismatches > 0:
        ...
        for i in range(sample_size):
            ...
            print(f"  [{row:4d}, {col:4d}]: result={result_val:12.6f}, expected={expected_val:12.6f}, diff={diff:12.6f}")
    ...
    print(f"First half cols [0-{half_n}): mismatches={first_half_mismatches}, unexpected_zeros={first_half_zeros}")
    print(f"Second half cols [{half_n}-{N}): mismatches={second_half_mismatches}, unexpected_zeros={second_half_zeros}")
