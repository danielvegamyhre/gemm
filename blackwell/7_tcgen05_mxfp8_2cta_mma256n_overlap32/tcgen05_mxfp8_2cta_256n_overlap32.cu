...
void consumer_warp(
    ...
    constexpr int NUM_EPILOGUE_MBARS = 7; // 7 epilogue mbars for chunks: 128, 64, 32, 32, 32, 64, 128
    ...
    int epilogue_parity_0 = 0, epilogue_parity_1 = 0, epilogue_parity_2 = 0, epilogue_parity_3 = 0;
    int epilogue_parity_4 = 0, epilogue_parity_5 = 0, epilogue_parity_6 = 0;
    ...
    for (int group_id = start_group_id; group_id < total_groups; group_id += num_groups) {
        ...
        {
            // after first block, always wait for epilogue to finish with the overlapped 32 columns (mbar 3)
            mbarrier_wait_parity(epilogue_mbar_addr + 3 * sizeof(uint64_t), epilogue_parity_3);
            epilogue_parity_3 ^= 1;

            // after 2+ blocks:
            // - mma buf 0 needs to wait for epilogue to finish with chunks 0, 1, 2
            // - mma buf 1 needs to wait for epilogue to finish with chunks 4, 5, 6
            if (blocks_processed >= 2)
            {
                if (mma_tmem_buf == 0)
                {
                    // Wait for leftmost chunks of buf 0: 128, 64, 32 cols (mbars 0, 1, 2)
                ...
                {
                    // Wait for rightmost chunks of buf 1: 32, 64, 128 cols (mbars 4, 5, 6)
                    mbarrier_wait_parity(epilogue_mbar_addr + 4 * sizeof(uint64_t), epilogue_parity_4);
                    epilogue_parity_4 ^= 1;

                    mbarrier_wait_parity(epilogue_mbar_addr + 5 * sizeof(uint64_t), epilogue_parity_5);
                    epilogue_parity_5 ^= 1;

                    mbarrier_wait_parity(epilogue_mbar_addr + 6 * sizeof(uint64_t), epilogue_parity_6);
                    epilogue_parity_6 ^= 1;
                }
            }
        }
...
void epilogue_warpgroup(
    ...
    for (int group_id = start_group_id; group_id < total_groups; group_id += num_groups) {
        ...
        // MMA buf 0: [128, 64, 32, 32] at offsets [0, 128, 192, 224], process right to left -> mbars 0, 1, 2, 3
        // MMA buf 1: [32, 32, 64, 128] at offsets [0, 32, 64, 128], process left to right -> mbars 3, 4, 5, 6
        if (mma_tmem_buf == 0) {
            process_epilogue_chunk<32, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 224, epilogue_mbar_addr + 3 * sizeof(uint64_t), cta_rank);
            process_epilogue_chunk<32, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 192, epilogue_mbar_addr + 2 * sizeof(uint64_t), cta_rank);
            process_epilogue_chunk<64, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 128, epilogue_mbar_addr + 1 * sizeof(uint64_t), cta_rank);
            process_epilogue_chunk<128, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 0, epilogue_mbar_addr + 0 * sizeof(uint64_t), cta_rank);
        } else {
            process_epilogue_chunk<32, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 0, epilogue_mbar_addr + 3 * sizeof(uint64_t), cta_rank);
            process_epilogue_chunk<32, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 32, epilogue_mbar_addr + 4 * sizeof(uint64_t), cta_rank);
            process_epilogue_chunk<64, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 64, epilogue_mbar_addr + 5 * sizeof(uint64_t), cta_rank);
            process_epilogue_chunk<128, BM, BN>(C, N, c_row_base, c_col_base, tmem_addr_reg, tmem_base_row, 128, epilogue_mbar_addr + 6 * sizeof(uint64_t), cta_rank);
        }
...
void ws_gemm_2cta_mma(
    ...
    constexpr int NUM_EPILOGUE_TMEM_BUFFERS = 7; // two 256 col wide accumulators with 32 cols overlapping = 7 chunks (128,64,32,32,32,64,128)
    ...
    constexpr int ACCUM_OVERLAP_COLS = 32;
