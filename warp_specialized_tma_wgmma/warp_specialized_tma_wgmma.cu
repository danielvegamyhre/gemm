...
__device__ __forceinline__ void wgmma_m64n16k16(uint64_t smem_desc_a, uint64_t smem_desc_b, float reg_c[1][8]) {
    ...
    asm volatile(
        ...
        "%10, %11, %12, %13, %14;\n"
        "}\n"
        : "+f"(reg_c[0][0]), "+f"(reg_c[0][1]), "+f"(reg_c[0][2]), "+f"(reg_c[0][3]),
          "+f"(reg_c[0][4]), "+f"(reg_c[0][5]), "+f"(reg_c[0][6]), "+f"(reg_c[0][7])
